...
export const GET = handle(async ({ userId, services, request }) => {
  ...
  const [totals, breakdown, netWorth, cashFlow, needsReviewCount, accounts] = await Promise.all([
  ...
  return { month, totals, breakdown, netWorth, cashFlow, needsReviewCount, accounts };
});
