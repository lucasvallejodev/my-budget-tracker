...
Seeded once per user by `ensureUserBootstrap` from `src/server/categories/default-taxonomy.ts` (version 1). Icons are names from the curated registry in `src/components/icons/registry.ts`. Everything below can be renamed, recoloured, reordered, extended or archived by the user; the Income group is the only one that cannot be archived.
