...
4. **Edit** changes the limit; **Delete** removes the limit for that month only (transactions are untouched; setting a limit for the same category, month and currency again brings the deleted budget back); **View transactions** jumps to the Transactions page filtered by the category name.
