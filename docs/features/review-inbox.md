...
- The page (`src/components/finance/review-inbox.tsx`) lists `/api/transactions?needsReview=1` and calls `categorizeTransactionAction`, which sets the category and `needs_review = false` through `ledger.updateStandard`.
