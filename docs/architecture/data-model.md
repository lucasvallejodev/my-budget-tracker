...
The schema lives in `src/db/schema.ts`; the SQL that created it is `drizzle/0001_ledger.sql`. All tables use text UUID primary keys generated by the application, `created_at`/`updated_at` timestamps with time zone, and a `user_id` column holding the Clerk user id (except the global `currencies` table).
