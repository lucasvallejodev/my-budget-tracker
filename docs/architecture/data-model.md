...
The schema lives in `apps/api/src/db/schema.ts` (the API owns the database); the SQL that creates it is `apps/api/drizzle/0000_init.sql`. The web app still carries a transitional copy (`apps/web/src/db/schema.ts`, `apps/web/drizzle/`) without the user tables and the extra soft-delete columns until it is switched to the API. All tables use text UUID primary keys generated by the application, `created_at`/`updated_at` timestamps with time zone, and a `user_id` column holding the Clerk user id (except the global `currencies` table).

![Target data model](../assets/diagrams/data-model-er.svg)

## Users, sessions and deletion
