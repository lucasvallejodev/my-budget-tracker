...
The schema lives in `apps/api/src/db/schema.ts` (the API owns the database); the SQL that creates it is `apps/api/drizzle/0000_init.sql`. All tables use text UUID primary keys generated by the application, `created_at`/`updated_at` timestamps with time zone, and a `user_id` column referencing `users(id)` (except the global `currencies` table and `users` itself).
