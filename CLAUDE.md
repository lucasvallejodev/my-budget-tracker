...
| `apps/web/`, `packages/shared/`        | npm workspaces: the Next.js app (`src/`, `drizzle/` migrations, `scripts/` DB check) and the code shared by client and server (Zod contracts, money/date/CSV helpers, constants). See `agents/architecture.md`. |
