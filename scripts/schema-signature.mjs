const tables = ['Account', 'Payee', 'Transaction', 'MonthlyHistory', 'MonthlyCategoryGroupHistory'];
const signatures = [
  ...
  `SELECT t.typname AS name, e.enumlabel AS value FROM pg_enum e JOIN pg_type t ON t.oid=e.enumtypid JOIN pg_namespace n ON n.oid=t.typnamespace WHERE n.nspname='public' AND t.typname IN ('AccountType','TransactionType') ORDER BY t.typname,e.enumsortorder`,
];

export async function readSchemaSignature(database) {
  const result = [];
  for (let i = 0; i < signatures.length; i++) {
    result.push((await database.query(signatures[i], i === 3 ? [] : [tables])).rows);
  }
  return result;
}
